...
class PhonemeCorrector:
    ...
    def __init__(self, threshold: float = 0.7, similar_threshold: float = None,
        ...
        self.accu_rag = AccuRAG(threshold=min(self.threshold, self.similar_threshold))
    ...
    def correct(self, text: str, k: int = 10) -> CorrectionResult:
        ...
        with self._lock:
            ...
            if fast_results:
                ...
                merged_results = []
                for hw, fast_score in fast_results:
                    accu_score = accu_scores.get(hw, fast_score)
                    best_score = max(fast_score, accu_score)
            ...
            # Stage 3: Fine-grained substring matching with word boundary constraints
